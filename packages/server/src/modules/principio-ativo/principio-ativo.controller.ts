...
export class PrincipioAtivoController {
  ...
  async addConcentracao(req: Request, res: Response) {
    const result = await service.addConcentracao(req.params.id, req.body);
    res.status(201).json({ success: true, data: result });
  }

  async updateConcentracao(req: Request, res: Response) {
    const result = await service.updateConcentracao(req.params.id, req.params.cid, req.body);
    res.json({ success: true, data: result });
  }

  async deleteConcentracao(req: Request, res: Response) {
    await service.deleteConcentracao(req.params.id, req.params.cid);
    res.json({ success: true, message: 'Concentracao removida' });
  }

  // Faixas
  async addFaixa(req: Request, res: Response) {
    const result = await service.addFaixa(req.params.id, req.body);
    res.status(201).json({ success: true, data: result });
  }

  async updateFaixa(req: Request, res: Response) {
    const result = await service.updateFaixa(req.params.id, req.params.fid, req.body);
    res.json({ success: true, data: result });
  }

  async deleteFaixa(req: Request, res: Response) {
    await service.deleteFaixa(req.params.id, req.params.fid);
